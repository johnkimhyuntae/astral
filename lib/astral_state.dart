...
class AstralState extends ChangeNotifier {
  ...
  Map<WType, String> statIdAboutMap = {
    ...
    WType.feltTemperature:
        "The temperature is the average over a short period of time. The 'Feels Like' temperature is the temperature perceived by humans, taking into account wind speed and humidity.",
    WType.cloudPercentage:
        "The cloud cover is the fraction of the sky covered by clouds. A value of 0% means the sky is clear, while 100% means the sky is completely overcast.",
    WType.fogPercentage:
        "The fog percentage is the likelihood of fog forming. A value of 0% means no fog, while 100% means fog is certain.",
    WType.humidity:
        "The humidity is the amount of water vapour in the air. A value of 0% means the air is completely dry, while 100% means the air is completely saturated.",
    WType.precipitation:
        "The precipitation is the amount of rain or snow that falls in a given time period. A value of 0mm means no precipitation, while 100mm means a very heavy downpour.",
    WType.precipitationPercentage:
        "The precipitation percentage is the likelihood of precipitation occurring. A value of 0% means no precipitation, while 100% means precipitation is certain.",
    WType.sunshineMinutes:
        "The sunshine minutes is the amount of time the sun is visible in a given time period. A value of 0 minutes means no sunshine, while 60 minutes means the sun is visible for the entire hour.",
    WType.temperature:
        "The temperature is the average over a short period of time. It is measured in degrees Celsius.",
    WType.uvIndex:
        "The UV index is a measure of the strength of the sun's ultraviolet radiation. A value of 0 means low risk of harm from unprotected sun exposure, while 11+ means extreme risk of harm.",
    WType.visibility:
        "The visibility is the distance at which objects can be clearly seen. A value of 0km means no visibility, while 10km means excellent visibility.",
    WType.windDirection:
        "The wind direction is the direction from which the wind is blowing. It is measured in compass directions.",
    WType.moonrise:
        "The moonrise is the time at which the moon first becomes visible above the horizon.",
    WType.moonset:
        "The moonset is the time at which the moon last becomes visible above the horizon.",
    WType.moonPhase:
        "The moon phase is the shape of the illuminated portion of the moon as seen from Earth. The different phases are new moon, waxing crescent, first quarter, waxing gibbous, full moon, waning gibbous, last quarter, and waning crescent.",
    WType.sunrise:
        "The sunrise is the time at which the sun first becomes visible above the horizon.",
    WType.sunset:
        "The sunset is the time at which the sun last becomes visible above the horizon.",
  };
  ...
  void parseAstro<T>(WType type, List daily) {
    statIdValueMapDailyAverage[type] = daily.map((e) => (e as String)).toList();
  }
  ...
  void parseType<T>(
    ...
    statIdValueMapHourly[type] =
        data.map((day) => day.map((e) => "${parse(e)}$unit").toList()).toList();
    if (daily != null) {
          ...
          builder: (context) => Center(
                child: Material(
                  ...
                  child: AlertDialog(
                    ...
                    content: Text('We were not able to find\n"$newLocation".\nPlease try a different location.'),
      ...
      for (int i = 1; i < val.length; i++) {
        if ((val.substring(i, i + 1) == " ") &&
          ...
          val = val.substring(0, i + 1) +
              val.substring(i + 1, i + 2).toUpperCase() +
  ...
  void updateWeather() {
    ...
    APIHandler.fetchWeatherData(lat, lon).then((weatherData) {
      ...
      parseType(
        (value) {
          return ((value as double) * 3.6)
              .toStringAsPrecision(3); // ms^-1 -> kmph
        },
        " km/h",
        WType.windSpeed,
      ...
      parseAstro(
        WType.moonPhase,
        turnToCapitalAtStart(weatherData.dailyData.moonphasename
            .map((x) => x as String)
